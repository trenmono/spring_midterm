...
(function () {
  var canvas = document.getElementById('adminSalesPieChart');
  if (!canvas || typeof adminCategorySalesData === 'undefined' || !adminCategorySalesData.length) return;
  ...
  var labels = adminCategorySalesData.map(function (d) { return d.label; });
  var values = adminCategorySalesData.map(function (d) { return d.value; });
  ...
  var legend = document.getElementById('adminPieLegend');
  if (legend) {
    var total = values.reduce(function (a, b) { return a + b; }, 0);
    adminCategorySalesData.forEach(function (d, i) {
